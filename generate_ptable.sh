...
case ${PTABLE} in
  ...
  aosp-8g|aosp-16g)
    ...
    #debug to enlarge 80M for compatible with old partition table from hisilicon
    fakeroot ${SGDISK} -n 3:0:+81M -t 3:0700 -u 3:61A36FC1-8EFB-4899-84D8-B61642EFA723 -c 3:"mcuimage" ${TEMP_FILE}
    ...
    #[6: fip: 340M-352M]
    fakeroot ${SGDISK} -n 6:0:+12M -t 6:0700 -u 6:17cc2359-dad2-4214-8362-1bfebc9a85fd -c 6:"fip" ${TEMP_FILE}
    #[7: dtimage: 352M-374M]
    fakeroot ${SGDISK} -n 7:0:+16M -t 7:0700 -u 7:A092C620-D178-4CA7-B540-C4E26BD6D2E2 -c 7:"dtimage" ${TEMP_FILE}
    #[8: trustfirmware: 374M-376M]
    fakeroot ${SGDISK} -n 8:0:+2M -t 8:0700 -u 8:5C0F213C-17E1-4149-88C8-8B50FB4EC70E -c 8:"trustfirmware" ${TEMP_FILE}
    #[9: system: 376M-5058M]
    fakeroot ${SGDISK} -n 9:0:+4688M -t 9:8300 -u 9:BED8EBDC-298E-4A7A-B1F1-2500D98453B7 -c 9:"system" ${TEMP_FILE}
    #[10: vendor: 5058M-5842M]
    fakeroot ${SGDISK} -n 10:0:+784M -t 10:0700 -u 10:FC56E345-2E8E-49AE-B2F8-5B9D263FE377 -c 10:"vendor" ${TEMP_FILE}
    #[11: userdata: 5830M-End]
    fakeroot ${SGDISK} -n -E -t 11:8300 -u 11:064111F6-463B-4CE1-876B-13F3684CE164 -c 11:"userdata" -p -D ${TEMP_FILE}
    ;;
  linux*)
    dd if=/dev/zero of=${TEMP_FILE} bs=${SECTOR_SIZE} count=${SECTOR_NUMBER} conv=sparse
    fakeroot ${SGDISK} -U 2CB85345-6A91-4043-8203-723F0D28FBE8 -v ${TEMP_FILE}
