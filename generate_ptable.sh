...
case ${PTABLE} in
  ...
  aosp*)
    ...
    #[3: fastboot: 10M-24M]
    fakeroot sgdisk -n 3:0:+12M -t 3:0700 -u 3:496847AB-56A1-4CD5-A1AD-47F4ACF055C9 -c 3:"fastboot" ${TEMP_FILE}
    #[4: fip: 24M-36M]
    fakeroot sgdisk -n 4:0:+12M -t 4:EF02 -u 4:b28add62-da27-4dd2-8dea-e3628a513929 -c 4:"fip" ${TEMP_FILE}
    #[5: nvme: 36M-38M]
    fakeroot sgdisk -n 5:0:+2M -t 5:0700 -u 5:00354BCD-BBCB-4CB3-B5AE-CDEFCB5DAC43 -c 5:"nvme" ${TEMP_FILE}
    #[6: optee: 38M-102M]
    fakeroot sgdisk -n 6:0:+64M -t 6:0700 -u 6:61b94129-c1f2-4601-99ea-a1f518e1b082 -c 6:"optee" ${TEMP_FILE}
    #[7: cache: 102M-358M]
    fakeroot sgdisk -n 7:0:+256M -t 7:8301 -u 7:A092C620-D178-4CA7-B540-C4E26BD6D2E2 -c 7:"cache" ${TEMP_FILE}
    #[8: boot: 358M-422M]
    fakeroot sgdisk -n 8:0:+64M -t 8:EF00 -u 8:5C0F213C-17E1-4149-88C8-8B50FB4EC70E -c 8:"boot" ${TEMP_FILE}
    #[9: reserved: 422M-678M]
    fakeroot sgdisk -n 9:0:+256M -t 9:0700 -u 9:BED8EBDC-298E-4A7A-B1F1-2500D98453B7 -c 9:"reserved" ${TEMP_FILE}
    #[10: system: 678M-2726M]
    fakeroot sgdisk -n 10:0:+2048M -t 10:8300 -u 10:FC56E345-2E8E-49AE-B2F8-5B9D263FE377 -c 10:"system" ${TEMP_FILE}
    #[11: userdata: 2726M-End]
    fakeroot sgdisk -n -E -t 11:8300 -u 11:064111F6-463B-4CE1-876B-13F3684CE164 -c 11:"userdata" -p ${TEMP_FILE}
    ;;
  linux*)
    dd if=/dev/zero of=${TEMP_FILE} bs=${SECTOR_SIZE} count=${SECTOR_NUMBER} conv=sparse
    fakeroot sgdisk -U 2CB85345-6A91-4043-8203-723F0D28FBE8 -v ${TEMP_FILE}
    ...
    fakeroot sgdisk -n 9:0:+256M -t 9:0700 -u 9:BED8EBDC-298E-4A7A-B1F1-2500D98453B7 -c 9:"reserved" ${TEMP_FILE}
    #[10: system: 678M-End]
    fakeroot sgdisk -n -E -t 10:8300 -u 10:FC56E345-2E8E-49AE-B2F8-5B9D263FE377 -c 10:"system" ${TEMP_FILE}
    ;;
esac
