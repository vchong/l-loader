...
"hi3660-hikey960.dtb")
	if [ -f $OUT/$BOOT/Image ]; then
		#UTILS=./utils
		TOOLS=${ANDROID_BUILD_TOP}/optee/tools-images-hikey960
		#ramdisk shell
		#RAMDISK=$UTILS/ramdisk_64.img
		#android ramdisk
		#RAMDISK=$UTILS/ramdisk.img
		...
		abootimg --create $OUT/$BOOT/boot.img -k $OUT/$BOOT/Image-dtb -r $RAMDISK -f ${ANDROID_BUILD_TOP}/optee/l-loader/bootimg-960.cfg
		cp $OUT/$BOOT/boot.img $DIR/boot.img
		cp $OUT/$BOOT/boot.img ${ANDROID_BUILD_TOP}/device/linaro/hikey/installer/hikey960/boot.img
		if [ -f $TOOLS/dtbTool ]; then
			...
			cp dt.img ${ANDROID_BUILD_TOP}/device/linaro/hikey/installer/hikey960/dt.img
		fi
	fi
	;;
"hip04-d01.dtb" )
